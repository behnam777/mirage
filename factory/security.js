...
security.signature = async (fieldValue, Model, FieldName) => {//field is data like {id:idValue} must be in model
    ...
    if (!(global.Models[model])) { return { state: false, message: 'data model not found', signatures: [] }; }
        ...
        const accessToken = await jwt.sign({ data: fieldValue }, security.salt, { expiresIn: process.env.AuthenticationAccessTime });
        const refreshToken = await jwt.sign({ data: fieldValue }, security.salt, { expiresIn: process.env.AuthenticationRefreshTime });
        Model.findOneAndUpdate({ [fieldName]: fieldValue }, { $set: { jwtTokens: [token1, token2] } }, { new: true }, (err, entity) => {
            ...
            else { return { state: true, message: 'entity jwt token updated', accessToken: accessToken, refreshToken: refreshToken } }
        })
    }
}
//******************************************************************************************
security.verify = async (signature, refresh, Model, FieldName) => {
    ...
    if (!(global.Models[model])) { return { state: false, message: 'data model not found', entity: '' }; }
        ...
        if (refresh) {
            security.signature(model, fieldName, fieldValue).then((result) => {
                return { state: true, message: 'verified successfully and tokens are refresh', entity: entity, accessToken: result.accessToken, refreshToken: result.refreshToken };
